...
describe("Marketplace unavailable surfaces", () => {
  ...
  test("renders the complete Prompt inventory without inventing a prompt", () => {
    ...
    for (const text of [
      "Prompt catalog is unavailable from the current contract",
      "What it gives you",
      "Preview or example",
      "Use when",
      "Do not use when",
  ...
  test("renders the complete Component inventory without a synthetic specimen", () => {
    ...
    for (const text of [
      "Component catalog is unavailable from the current contract",
  ...
  test("renders the complete Skill trust inventory without claiming runs, files, or permissions", () => {
    ...
    for (const text of [
      "Skill catalog is unavailable from the current contract",
  ...
  test("wires category, detail Back, and collection routes through Marketplace history callbacks", async () => {
    const host = createReactHost();
    const root = createRoot(host.container as unknown as Element);
    ...
    try {
      await act(async () => root.render(<MarketplaceScreenView catalog={null} location={categoryLocation} sidebarVisible snapshot={snapshot()} onBack={back} onNavigate={navigate} onRememberLocation={() => undefined} onRetry={() => undefined} />));
      ...
      await act(async () => root.render(<MarketplaceScreenView catalog={null} location={{ ...categoryLocation, route: { kind: "unavailable-detail", category: "skills" } }} sidebarVisible snapshot={snapshot()} onBack={back} onNavigate={navigate} onRememberLocation={() => undefined} onRetry={() => undefined} />));
      ...
      await act(async () => root.render(<MarketplaceScreenView catalog={null} location={{ ...categoryLocation, route: { kind: "collection" } }} sidebarVisible snapshot={snapshot()} onBack={back} onNavigate={navigate} onRememberLocation={() => undefined} onRetry={() => undefined} />));
