...
export async function auditInstrumentSource({ productionDist = resolve(root, "dist") } = {}) {
  ...
  for (const path of sourceFiles) {
    ...
    if (name !== "src/instrument/palette.ts" && name !== "src/styles/tokens.css" && /#[\da-f]{3,8}\b/i.test(text)) violations.push(`${name}: direct color literal`);
    if (text.includes('from "@radix-ui/react-dialog"') && name !== "src/instrument/overlay-registry.tsx" && !text.includes("data-instrument-overlay")) violations.push(`${name}: raw dialog lacks Instrument registry marker`);
  }
