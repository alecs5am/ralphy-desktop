...
export function DynamicIsland({ feed, context, projectName, mock, onNavigate }: {
  ...
  const taskTone = feed.activeTask?.status === "failed" ? "text-alert" : "text-on-instrument-muted-decorative";
  ...
  return <div className="dynamic-island" ref={root} data-open={open || undefined} data-mock={mock || undefined} data-animate={animate || undefined}>
    <div className="dynamic-island-shell">
      ...
      >
        <span className="dynamic-island-context" key={contextLabel} style={context.identity ? projectGlyphVars(context.identity) : undefined}>
          {context.identity
            ? <i className="dynamic-island-identity" style={{ maskImage: `url("${projectGlyphAsset(context.identity)}")`, WebkitMaskImage: `url("${projectGlyphAsset(context.identity)}")` }} aria-hidden="true" />
            : <i className="dynamic-island-identity is-blank" aria-hidden="true" />}
          <span className="dynamic-island-context-label">{contextLabel}</span>
          {context.count !== null && <b className="dynamic-island-context-count">{context.count}</b>}
        </span>
        ...
        {feed.activeTask && <span className="dynamic-island-task flex min-w-0 items-center gap-2">
          <i className="dynamic-island-orb size-4 shrink-0" aria-hidden="true" />
      ...
      >
        <div className="dynamic-island-detail-inner">
          ...
          {projectName && feed.projectStatus.status === "unavailable" && <p className="m-0 rounded-2xl bg-instrument-raised px-3 py-2 type-xs text-on-instrument-muted">{feed.projectStatus.reason}</p>}
          {feed.activeTask && <section className="grid gap-1" aria-label="Active task">
            ...
            <button className="grid min-h-14 grid-cols-[28px_minmax(0,1fr)_auto] items-center gap-2 rounded-2xl bg-instrument-raised px-2.5 py-2 text-left text-on-instrument hover:bg-instrument-hover focus-visible:outline-2 focus-visible:outline-offset-[-2px] focus-visible:outline-on-instrument aria-disabled:text-on-instrument-muted" type="button" aria-disabled={!feed.activeTask.destination} onClick={() => { if (feed.activeTask?.destination) onNavigate(feed.activeTask.destination); }}>
              <span className="grid size-7 place-items-center rounded-full bg-instrument">{feed.activeTask.status === "running" ? <LoaderCircle className="is-spinning" aria-hidden="true" size={14} /> : feed.activeTask.status === "failed" ? <CircleAlert className="text-alert" aria-hidden="true" size={14} /> : <CircleCheck className="text-on-instrument-muted" aria-hidden="true" size={14} />}</span>
          ...
          {!feed.activeTask && <section className="grid gap-1" aria-label="Active task">
            <span className="px-2 font-code type-mono-xs tracking-mono text-on-instrument-muted">ACTIVE TASK</span>
            <p className="m-0 rounded-2xl bg-instrument-raised px-3 py-2.5 type-xs text-on-instrument-muted">{projectName ? "No active task for this project." : "Open a project to track its active task here."}</p>
          </section>}
          <section className="grid gap-1" aria-label="Notifications">
            ...
            {notifications.length === 0 && <p className="m-0 rounded-2xl bg-instrument-raised px-3 py-2.5 type-xs text-on-instrument-muted">{feed.notifications.status === "empty" || feed.notifications.status === "unavailable" ? feed.notifications.reason : "No notifications."}</p>}
          </section>
        </div>
      </div>
    </div>
