...
function StreamingAudioPlayer({ src, name, compact, probing, onMetadata, onError }: { src: string; name: string; compact: boolean; probing: boolean; onMetadata?(duration: number): void; onError?(): void }) {
  ...
  const content = <><div className={`audio-waveform-heading${compact ? " justify-center gap-2.25 [&>span]:max-w-[calc(100%_-_42px)]" : ""}`}><button className={`audio-play-button${compact ? " size-8" : ""}`} type="button" aria-label={`${playing ? "Pause" : "Play"} ${name}`} disabled={!ready || probing} onClick={toggle}>{playing ? <Pause size={compact ? 16 : 21} fill="currentColor" /> : <Play size={compact ? 16 : 21} fill="currentColor" />}</button><span><strong className={compact ? "hidden" : undefined}>{name}</strong><small className={compact ? "type-xs" : undefined}>{probing ? "Checking audio…" : error ? "Preview unavailable" : ready ? `${formatTime(duration)} · streaming preview` : "Loading audio…"}</small></span></div>
    <div className={`audio-stream-waveform${compact ? " h-14" : ""}`}><div className={`audio-stream-bars absolute inset-0 flex items-center overflow-hidden ${compact ? "gap-0.5" : "gap-0.75"}`} aria-hidden="true">{bars.map((height, index) => <i className={`min-w-px flex-1 rounded-control [transition:background_var(--dur-fast)_linear] motion-reduce:[transition:none] ${index / bars.length <= progress ? "is-played bg-on-instrument" : "bg-on-instrument-muted"}`} key={index} style={{ height: `${height}%` }} />)}</div><SnappySlider className={STREAM_SEEK} value={currentTime} min={0} max={Math.max(duration, 0.1)} step={0.1} ariaLabel={`Position in ${name}`} disabled={!ready || probing} onValueChange={seek} /></div></>;
  return <div className={`audio-waveform-player audio-stream-player${compact ? " is-compact size-full justify-center gap-0 p-3" : ""}`}>
...
function DecodedAudioWaveform({ src, name, compact, onReady, onFallback }: { src: string; name: string; compact: boolean; onReady?(): void; onFallback(): void }) {
  ...
  const content = <><div className={`audio-waveform-heading${compact ? " justify-center gap-2.25 [&>span]:max-w-[calc(100%_-_42px)]" : ""}`}><button className={`audio-play-button${compact ? " size-8" : ""}`} type="button" aria-label={`${playing ? "Pause" : "Play"} ${name}`} disabled={!ready} onClick={toggle}>{playing ? <Pause size={compact ? 16 : 21} fill="currentColor" /> : <Play size={compact ? 16 : 21} fill="currentColor" />}</button><span><strong className={compact ? "hidden" : undefined}>{name}</strong><small className={compact ? "type-xs" : undefined}>{ready ? `${formatTime(duration)} audio` : "Building waveform…"}</small></span></div>
  ...
  return <div className={`audio-waveform-player${compact ? " is-compact size-full justify-center gap-0 p-3" : ""}`} aria-label={name}>
    {compact ? <div className="audio-compact-content grid w-full gap-3">{content}</div> : content}
