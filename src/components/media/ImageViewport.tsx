...
export function ImageViewport({ src, name, compact = false }: ImageViewportProps) {
  ...
  useEffect(() => {
    ...
    setTransform((current) =>
      clampImageTransform(current, fittedSize, viewportSize));
  }, [
    fittedSize.height,
    fittedSize.width,
    viewportSize.height,
  ...
  const geometry = () => {
    ...
    return {
      bounds,
      viewport,
      image: containedImageSize(naturalSize, viewport),
    };
  };
  ...
  const setScale = (scale: number, point = { x: 0, y: 0 }) => {
    ...
    setTransform((current) =>
      clampImageTransform(
        zoomAroundPoint(current, point, scale),
        sizes.image,
        sizes.viewport,
      ));
  };
  ...
  const onWheel = (event: WheelEvent<HTMLDivElement>) => {
    ...
    if (!sizes) return;
    const point = {
      x: event.clientX - sizes.bounds.left - sizes.bounds.width / 2,
      y: event.clientY - sizes.bounds.top - sizes.bounds.height / 2,
    };
    setTransform((current) =>
      clampImageTransform(
        zoomAroundPoint(current, point, scaleFromWheel(current.scale, event.deltaY)),
        sizes.image,
        sizes.viewport,
      ));
  };
  ...
  return (
    <div
      className={`image-viewport${compact ? " is-compact" : ""}${
        panning ? " is-panning" : ""
      }`}
      ref={rootRef}
      onWheel={onWheel}
      ...
      {!compact && (
        <div className="image-zoom-controls">
          <button
            ...
            onClick={() => setScale(transform.scale / 1.35)}
          >
            <ZoomOut size={15} />
          </button>
