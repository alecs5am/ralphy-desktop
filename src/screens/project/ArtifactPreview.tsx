...
import { COMMAND_BUTTON, PREVIEW_UNAVAILABLE, PROJECT_SKELETON } from "../route-chrome";
...
export function ArtifactPreview({ preview, empty, retry }: { preview: Preview; empty: string; retry(): void }) {
  ...
  if (preview.status === "loading") return <div className={PROJECT_SKELETON} role="status">Loading preview…</div>;
  if (preview.status === "error") return <div className={PREVIEW_UNAVAILABLE} role="alert"><ImageOff size={20} aria-hidden="true" /><strong>Preview unavailable</strong><span>{preview.error}</span><button className={COMMAND_BUTTON} type="button" onClick={retry}>Retry</button></div>;
