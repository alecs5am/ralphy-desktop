...
export function CompositionsPanel({ page, controller, snapshot, pagination }: {
  ...
  return <div className="project-split-view composition-panel">
    ...
    <section className="project-preview composition-detail" aria-label="Composition detail">
      ...
      {snapshot.composition.status === "error" && <div className="project-local-error" role="alert"><AlertCircle size={17} aria-hidden="true" /><span>{snapshot.composition.error}</span><button type="button" onClick={() => { if (snapshot.compositionId) void controller.openComposition(snapshot.compositionId); }}>Retry</button></div>}
      {aggregate && <>
        <header className="composition-heading"><div><h3>{aggregate.slug}</h3><p>{aggregate.kind} · ID {aggregate.id}</p><p><strong>Selected {aggregate.selectedRevisionId ?? "None"}</strong> · <strong>Latest {aggregate.latestRevisionId ?? "None"}</strong></p></div><button type="button" disabled={pending || aggregate.latestRevisionId === null} onClick={() => { void controller.reviseSelectedComposition(); }}>{snapshot.compositionMutation === "revise" ? "Creating draft…" : "New draft"}</button></header>
        ...
        {inspected && <div className="composition-revision-detail">
          <header><div><h4>Revision {inspected.revisionNo}</h4><p>ID {inspected.id} · Parent {inspected.parentRevisionId ?? "None"} · Iteration {inspected.iterationId ?? "None"}</p><p>{inspected.engine}{inspected.engineVersion ? ` ${inspected.engineVersion}` : ""} · {inspected.state} · {inspected.authoredBySessionId ?? "No author Session"} · {formatTime(inspected.createdAt)}</p></div><div className="composition-actions"><button type="button" disabled={pending || inspected.state !== "sealed" || inspected.id === aggregate.selectedRevisionId} onClick={() => { void controller.selectInspectedCompositionRevision(); }}>{snapshot.compositionMutation === "select" ? "Selecting…" : "Make selected"}</button><button type="button" disabled={pending || inspected.state !== "draft" || inspected.id !== aggregate.latestRevisionId} onClick={() => { void controller.buildInspectedCompositionRevision(); }}>{snapshot.compositionMutation === "build" ? "Building…" : buildLabel(aggregate.kind)}</button></div></header>
