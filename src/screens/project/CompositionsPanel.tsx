...
const COMMAND = "min-h-control-sm px-3";
    ...
    <section className={`composition-detail ${PANE} block rounded-cell bg-surface-sunken px-4.5 pb-12 [&_.composition-empty]:text-muted [&_h4]:m-0 [&_h5]:m-0`} aria-label="Composition detail" ref={bindDetail} onScroll={detailScroll.onScroll}>
      ...
      {composition && <>
        ...
        {revision && <div className="composition-revision-detail [&>header]:flex [&>header]:items-start [&>header]:justify-between [&>header]:gap-3 [&>section]:mt-3 [&>section]:rounded-field [&>section]:bg-surface [&>section]:p-4 [&_p]:m-0 [&_p]:text-muted [&_p]:[overflow-wrap:anywhere]">
          <header><div><h4>Revision {revision.revisionNo}</h4><p>{revision.state} · {formatTime(revision.createdAt)}</p></div><div className="composition-actions flex gap-2"><button className={`command-button ${COMMAND}`} type="button" disabled={pending || revision.state !== "sealed" || revision.id === composition.selectedRevisionId} onClick={() => { void controller.selectInspectedCompositionRevision(); }}>{snapshot.compositionMutation === "select" ? "Selecting…" : "Make selected"}</button><button className={`command-button ${COMMAND}`} type="button" disabled={pending || revision.state !== "draft" || revision.id !== composition.latestRevisionId} onClick={() => { void controller.buildInspectedCompositionRevision(); }}>{snapshot.compositionMutation === "build" ? "Building…" : buildLabel(composition.kind)}</button></div></header>
          ...
          <section className="composition-primary [&_li]:mt-2 [&_li]:flex [&_li]:items-start [&_li]:justify-between [&_li]:gap-3 [&_p]:m-0 [&_p]:text-muted [&_small]:text-muted [&_span]:text-muted"><h5>{buildLabel(composition.kind)}</h5>{snapshot.compositionBuilds.status === "error" ? <div className="project-local-error" role="alert"><span>{snapshot.compositionBuilds.error}</span><button className={`command-button ${COMMAND}`} type="button" onClick={() => { void controller.loadMoreCompositionBuilds(); }}>Retry</button></div> : snapshot.inspectedCompositionBuild.status === "loading" ? <div role="status">Loading latest build…</div> : snapshot.inspectedCompositionBuild.status === "error" ? <div className="project-local-error" role="alert"><span>{snapshot.inspectedCompositionBuild.error}</span><button className={`command-button ${COMMAND}`} type="button" onClick={() => { void controller.inspectCompositionRevision(revision.id); }}>Retry</button></div> : build ? <><div className="composition-build-summary mb-2.5 flex items-baseline justify-between gap-3"><strong>{build.state}</strong><span>{formatTime(build.createdAt)}</span></div><Evaluations items={snapshot.compositionBuildEvaluations.items} /><PageTail root={detail} page={snapshot.compositionBuildEvaluations} load={() => { void controller.loadMoreCompositionBuildEvaluations(); }} /><ul>{sortPositioned(snapshot.compositionBuildOutputs.items).map((output) => <li key={output.id}><span>#{output.position} · {output.role ?? "output"}</span><button className={`command-button ${COMMAND}`} type="button" onClick={() => { void controller.previewCompositionOutput(output.artifactRevisionId); }}>Preview</button></li>)}</ul><PageTail root={detail} page={snapshot.compositionBuildOutputs} load={() => { void controller.loadMoreCompositionBuildOutputs(); }} /></> : <p>No builds yet.</p>}</section>
