...
function UnitCard({ unit, baseLifecycle, publications, controller, disabled, onOpen }: {
  ...
  return <article className="unit-card-shell group relative min-w-0 overflow-hidden rounded-cell bg-instrument text-on-instrument">
    <button className="unit-card flex w-full min-w-0 flex-col bg-transparent p-1.5 text-left text-on-instrument" type="button" disabled={disabled} aria-label={`Open ${unit.slug}`} onClick={(event) => onOpen(event.currentTarget)}>
      <span className="unit-card-preview relative aspect-video min-h-0 w-full overflow-hidden rounded-cell bg-instrument-raised"><CardMedia media={summary?.media ?? null} format={unit.format} /><em>{typeLabel(unit.format)}</em>{lifecycle.label === "Rendering" && <i className="unit-card-progress"><span /></i>}</span>
...
export function UnitsPanel({ page, controller, snapshot, targetUnitId, scrollMemory, resetToken }: {
  ...
  return <InstrumentScreenRoot descriptor={unitsInstrumentStates} state={unitsInstrumentState(page, snapshot, viewerOpen)}><div className="units-workbench units-grid-workbench">
    <div className="units-toolbar m-0 flex w-full max-w-none flex-wrap items-center gap-2 rounded-cell bg-surface-sunken p-2">
      ...
      <label className="units-search flex h-9 min-w-[200px] items-center gap-2 rounded-control bg-surface px-3"><Search className="size-4 text-muted" aria-hidden="true" /><input className="min-w-0 flex-1 bg-transparent type-base text-ink outline-none placeholder:text-muted" aria-label="Search units" placeholder="Search units" value={query} onChange={(event) => setQuery(event.target.value)} /></label>
    </div>
