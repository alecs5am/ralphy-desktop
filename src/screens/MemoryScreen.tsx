...
function EditorDialog({ open, entry, onOpenChange, onSave }: { open: boolean; entry?: MemoryDetailDto; onOpenChange(open: boolean): void; onSave(input: MemoryMutation): Promise<void> }) {
  ...
  return <MemoryModal open={open} onOpenChange={onOpenChange} title={entry ? "Revise memory" : "Add memory"} description={entry ? `Save as immutable version ${entry.version + 1}` : "Create a durable rule for future work"}>
    <form className="memory-editor" onSubmit={(event) => void submit(event)}>
      ...
      <div><label>Scope{entry ? <span className="memory-static-field">{tier === "workspace" ? "Workspace" : "Global"}</span> : <SelectMenu<MemoryTier> value={tier} options={[{ value: "workspace", label: "Workspace" }, { value: "global", label: "Global" }]} ariaLabel="Memory scope" onValueChange={setTier} />}</label><label>Type<SelectMenu<Exclude<MemoryType, "legacy">> value={type} options={TYPES.filter((value): value is Exclude<MemoryType, "legacy"> => value !== "legacy").map((value) => ({ value, label: TYPE_LABEL[value] }))} ariaLabel="Memory type" onValueChange={setType} /></label><label>State<SelectMenu<"active" | "proposed"> value={status} options={[{ value: "active", label: "Active" }, { value: "proposed", label: "Proposal" }]} ariaLabel="Memory state" onValueChange={setStatus} /></label></div>
