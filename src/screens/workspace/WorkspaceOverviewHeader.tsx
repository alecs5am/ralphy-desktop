...
export function WorkspaceOverviewHeader({
  ...
  return <header className="screen-header workspace-overview-header relative m-0 flex min-h-0 w-full max-w-none flex-wrap items-center justify-between gap-4 rounded-panel border-0 bg-instrument px-5 py-4 text-on-instrument shadow-none">
    <div className="min-w-0">
      <div className="screen-kicker text-[11px] uppercase tracking-[0.14em] text-on-instrument-muted">Workspace overview</div>
      <h1 className="mt-1 truncate text-[28px] font-semibold leading-none tracking-[-0.03em] text-on-instrument">{value.name}</h1>
      {value.description && <p className="mt-1 text-[13px] leading-5 text-on-instrument-muted">{value.description}</p>}
      <div className="workspace-overview-meta mt-2 flex flex-wrap gap-x-3 gap-y-1 text-[11px] text-on-instrument-muted">
      ...
      {degraded.length > 0 && <p className="workspace-overview-partial mt-2 text-[12px] text-on-instrument-muted"><strong className="text-on-instrument">Partial data</strong> · {degraded.join(" ")}</p>}
    </div>
    <div className="workspace-header-actions flex items-center gap-2 text-[12px] text-on-instrument-muted">
      <span className="rounded-full bg-instrument-raised px-3 py-2">{countLabel(criticalCount, "critical issue")}</span>
      <button className="command-button inline-flex min-h-9 items-center gap-2 rounded-control border-0 bg-surface px-3 text-[13px] font-medium text-ink" type="button" disabled={refreshing} onClick={onRefresh}>
