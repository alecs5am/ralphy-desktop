...
function RetryBanner({ title, reason, label, onRetry }: {
  ...
  return <div className="workspace-operation-banner" role="status">
...
function AttentionQueue({ value, onOpenPage, onRetry, expanded: controlledExpanded, onExpandedChange }: {
  ...
  return <section className="workspace-operations-panel workspace-attention" aria-labelledby="workspace-attention-heading">
...
function ProductionState({ value }: { value: OperationsValue["pulse"] }) {
  const available = value.status === "ready" || value.status === "partial";
  return <section className="workspace-operations-panel workspace-production-state" aria-labelledby="workspace-pulse-heading">
    <div className="workspace-section-heading"><h2 id="workspace-pulse-heading">Production pulse</h2><span>Lifecycle</span></div>
    <ul className="workspace-pulse-list" aria-label="Production lifecycle summary">
      {pulseStages.map((stage) => <li key={stage}><span aria-hidden="true">—</span><small>{stage}</small></li>)}
    </ul>
    {value.status !== "ready" && <div className="workspace-unavailable">
