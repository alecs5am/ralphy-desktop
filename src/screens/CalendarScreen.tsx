...
/* Vocabulary the Calendar repeats. Each string is complete: a surface never arrives without the
   ink it pairs with. */
const OVERLAY_PANEL = `absolute inset-y-0 right-0 z-header text-ink animate-calendar-panel-in motion-reduce:animate-none ${WINDOW}`;
...
function EventInspector({ event, postizAvailable, onClose, onOpenUnit, onMutate }: { event: CalendarEventDto; postizAvailable: boolean; onClose(): void; onOpenUnit(): void; onMutate(input: Parameters<typeof bridge.mutateCalendar>[1]): void }) {
  return <aside className={`calendar-inspector w-calendar-inspector ${OVERLAY_PANEL}`} data-instrument-overlay="calendar-inspector">
    <header className={WINDOW_TITLEBAR}><small className="font-code type-mono-md tracking-block text-muted">PUBLICATION</small><i className="flex-1" /><button type="button" className={`${PANEL_HEADER_ACTION} bg-surface-sunken text-ink hover:bg-surface-hover disabled:opacity-35`} disabled={event.projectId === null} onClick={onOpenUnit}><ArrowUpRight className={ICON} />Open Unit</button><button type="button" className={`${PANEL_HEADER_ACTION} bg-transparent text-muted hover:bg-surface-sunken hover:text-ink`} aria-label="Close inspector" onClick={onClose}><X className={ICON} /></button></header>
