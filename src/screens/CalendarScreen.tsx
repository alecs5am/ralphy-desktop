  ...
  return <CalendarWorkspaceContext.Provider value={workspaceId}><InstrumentScreenRoot descriptor={calendarInstrumentStates} state={instrumentState}><main className="main-region calendar-region flex min-h-0 flex-1 flex-col overflow-auto bg-transparent p-2 pb-6 text-[13px] text-ink">
    <section className="calendar-shell m-0 flex min-h-0 w-full max-w-none flex-1 flex-col gap-2 overflow-visible border-0 bg-transparent p-0 shadow-none" aria-busy={loading} aria-label={`${workspaceName} calendar`}>
      <header className="calendar-toolbar m-0 flex min-h-0 w-full max-w-none flex-wrap items-center gap-2 rounded-panel border-0 bg-instrument px-4 py-3 text-on-instrument shadow-none [&_button]:inline-flex [&_button]:min-h-8 [&_button]:items-center [&_button]:gap-1.5 [&_button]:rounded-control [&_button]:border-0 [&_button]:px-2.5 [&_button]:text-[12px]">
        ...
        <button type="button" className="calendar-primary bg-surface text-ink shadow-none" onClick={() => openSchedule()}><Plus />Schedule content</button>
      </header>
...
function FilterPopover({ data, filters, onChange, onClose }: { data: CalendarWorkspaceDto | null; filters: CalendarFilters; onChange(value: CalendarFilters): void; onClose(): void }) {
  ...
  return <div className="calendar-filter-popover rounded-[14px] border-0 bg-surface p-2 text-ink shadow-none" data-instrument-overlay="calendar-filter">
...
function MonthView({ days, events, timezone, selectedEventId, onOpen, onDropUnit }: { days: ReturnType<typeof monthDays>; events: CalendarEventDto[]; timezone: string; selectedEventId: string | null; onOpen(event: CalendarEventDto): void; onDropUnit(unitId: string, date: string): void }) {
  ...
  return <div className="calendar-month min-h-0 w-full overflow-hidden rounded-[14px] bg-surface">
    <div className="calendar-weekdays grid grid-cols-7 gap-[6px] bg-transparent text-center text-[10px] uppercase tracking-[0.12em] text-muted">{DOW.map((day) => <span className="py-1.5 pl-0" key={day}>{day}</span>)}</div>
    <div className="calendar-month-grid mt-[5px] grid grid-cols-7 gap-[6px] bg-transparent">{days.map((day) => {
      const items = events.filter((event) => event.at !== null && calendarDayKey(event.at, timezone) === day.key);
      return <div className={`calendar-month-cell min-h-24 min-w-0 overflow-hidden rounded-[14px] border-0 p-[7px] text-[11px] text-ink shadow-none${day.inMonth ? " bg-surface-sunken" : " is-outside bg-transparent"}${day.key === today ? " is-today bg-surface" : ""}`} key={day.key} tabIndex={0} onDragOver={(event) => { if (event.dataTransfer.types.includes(CALENDAR_UNIT_DRAG)) event.preventDefault(); }} onDrop={(event) => { const unitId = event.dataTransfer.getData(CALENDAR_UNIT_DRAG); if (unitId) onDropUnit(unitId, day.key); }}>
...
function CalendarEventButton({ event, timezone, lead = false, selected = false, onClick }: { event: CalendarEventDto; timezone: string; lead?: boolean; selected?: boolean; onClick(): void }) {
  return <button type="button" className={`calendar-event flex w-full min-w-0 items-center gap-1 overflow-hidden rounded-[7px] border-0 px-1.5 py-1 text-left text-[11px] leading-4 shadow-none${lead ? " is-lead" : ""}${selected ? " is-selected bg-instrument text-on-instrument [&_small]:text-on-instrument-muted" : " bg-surface text-ink"} is-${event.status}`} onClick={onClick}>{lead && <CalendarThumb event={event} className="h-9 w-[30px] rounded-[9px]" />}<span className="min-w-0"><b className="block truncate font-medium">{event.title}</b><small className="block truncate text-[9px] text-muted"><i />{formatCalendarTime(event.at, timezone)} · {event.channels.length} {event.channels.length === 1 ? "publication" : "publications"}</small></span>{eventStatusSummary(event) === "attention" && <AlertTriangle />}</button>;
}
...
function CalendarThumb({ event, className = "" }: { event: { id?: string; unitId?: string; projectId?: string | null; title: string; thumbnail?: { type: "artifact-revision"; id: string } | null }; className?: string }) {
  ...
  return <span className={`calendar-thumb border-0 shadow-none ${className}`} style={{ "--calendar-hue": String(hash(identity) % 360) } as CSSProperties} aria-hidden="true">{url && !failed ? <img src={url} alt="" onError={() => setFailed(true)} /> : <i>{event.title.slice(0, 1)}</i>}</span>;
}
...
function EventInspector({ event, postizAvailable, onClose, onOpenUnit, onMutate }: { event: CalendarEventDto; postizAvailable: boolean; onClose(): void; onOpenUnit(): void; onMutate(input: Parameters<typeof bridge.mutateCalendar>[1]): void }) {
  return <aside className="calendar-inspector rounded-panel border-0 bg-surface text-ink shadow-none [&_.calendar-date-card]:border-0 [&_.calendar-date-card]:bg-surface-sunken [&_.calendar-date-card]:shadow-none [&_.calendar-event-summary]:border-0 [&_.calendar-event-summary]:bg-surface-sunken [&_.calendar-event-summary]:shadow-none [&_.calendar-revision-card]:border-0 [&_.calendar-revision-card]:bg-surface-sunken [&_.calendar-revision-card]:shadow-none" data-instrument-overlay="calendar-inspector">
  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}>{open && <><Dialog.Overlay forceMount className="calendar-modal-overlay" data-instrument-overlay-backdrop="" /><Dialog.Content forceMount className="calendar-modal rounded-panel border-0 bg-surface text-ink shadow-none [&_button]:shadow-none [&_input]:border-0 [&_input]:bg-surface-sunken [&_input]:shadow-none [&_textarea]:border-0 [&_textarea]:bg-surface-sunken [&_textarea]:shadow-none" data-instrument-overlay="calendar-schedule">
    ...
    <div className="calendar-modal-layout">
      <aside className="calendar-modal-unit">
        ...
        {pickerOpen && <div className="calendar-unit-popover rounded-[14px] border-0 bg-surface-sunken shadow-none" data-instrument-overlay="calendar-unit-picker">{units.map((item) => <button type="button" key={item.unitId} className={item.unitId === unit?.unitId ? "is-selected bg-instrument text-on-instrument [&_small]:text-on-instrument-muted" : "bg-transparent text-ink"} onClick={() => { onSelect(item); setPickerOpen(false); }}><CalendarThumb event={item} /><span><strong>{item.title}</strong><small>{item.project} · R{item.revision ?? "—"} · {item.kind}</small></span>{item.unitId === unit?.unitId && <Check />}</button>)}</div>}
      </aside>
      {step === "content" ? <section className="calendar-schedule-form">
        ...
        <section className="calendar-channel-section"><header><span>CHANNELS</span><small>{selectedChannels.length} of {channels.filter((channel) => !disconnectedIds.has(channel.socialAccountId)).length} available selected{channels.some((channel) => disconnectedIds.has(channel.socialAccountId)) ? ` · ${channels.filter((channel) => disconnectedIds.has(channel.socialAccountId)).length} needs reconnect` : ""}</small><i /><button type="button" disabled={!unit} onClick={() => onStep("settings")}><SlidersHorizontal />Platform settings</button></header><div className="calendar-modal-channels">{channels.map((channel) => { const id = channelKey(channel); const Icon = platformIcon(channel.platform); const disconnected = disconnectedIds.has(channel.socialAccountId); const selected = selectedIds.includes(id); const content = <><AccountMark identity={`${channel.platform}:${channel.account}`} /><span><strong><Icon />{channel.account}</strong><small>{disconnected ? "Token expired — reconnect to publish" : `${capitalize(channel.platform)} · ready`}</small></span>{disconnected ? <button type="button" onClick={() => onReconnect(channel.socialAccountId)}>Reconnect</button> : <i className="calendar-check-box">{selected && <Check />}</i>}</>; return disconnected ? <div className="calendar-channel-option is-disconnected border-0 bg-surface-sunken shadow-none" key={id}>{content}</div> : <button type="button" className={`calendar-channel-option border-0 shadow-none ${selected ? "is-selected bg-instrument text-on-instrument [&_small]:text-on-instrument-muted" : "bg-surface-sunken text-ink"}`} key={id} onClick={() => toggleChannel(id)}>{content}</button>; })}</div></section>
...
function CalendarDatePicker({ value, onChange, onClose }: { value: string; onChange(value: string): void; onClose(): void }) {
  ...
  const today = localDateKey(new Date());
  return <div className="calendar-date-popover rounded-[14px] border-0 bg-surface text-ink shadow-none" data-instrument-overlay="calendar-date-popover" role="dialog" aria-label="Publication date" ref={ref} tabIndex={-1} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); onClose(); } }}>
    ...
    <div className="calendar-picker-days">{days.map((day) => <button type="button" className={`${day.inMonth ? "" : "is-outside"}${day.key === value ? " is-selected bg-instrument text-on-instrument" : ""}${day.key === today ? " is-today shadow-none" : ""}`} aria-label={`Choose ${new Intl.DateTimeFormat("en", { dateStyle: "full" }).format(day.date)}`} key={day.key} onClick={() => onChange(day.key)}>{day.date.getDate()}</button>)}</div>
...
function CalendarTimePicker({ value, onChange, onClose }: { value: string; onChange(value: string): void; onClose(): void }) {
  ...
  return <div className="calendar-time-popover rounded-[14px] border-0 bg-surface text-ink shadow-none" data-instrument-overlay="calendar-time-popover" role="dialog" aria-label="Publication time" ref={ref} tabIndex={-1} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); onClose(); } }}>
...
function ReconnectDialog({ account, credential, saving, onCredential, onOpenChange, onSave }: { account: CalendarWorkspaceDto["accounts"][number] | null; credential: string; saving: boolean; onCredential(value: string): void; onOpenChange(open: boolean): void; onSave(): void }) {
  return <Dialog.Root open={account !== null} onOpenChange={onOpenChange}>{account && <><Dialog.Overlay forceMount className="calendar-reconnect-overlay" data-instrument-overlay-backdrop="" /><Dialog.Content forceMount className="calendar-reconnect-dialog rounded-panel border-0 bg-surface text-ink shadow-none" data-instrument-overlay="calendar-reconnect">
