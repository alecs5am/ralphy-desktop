...
function MonthView({ days, events, timezone, selectedEventId, onOpen, onDropUnit }: { days: ReturnType<typeof monthDays>; events: CalendarEventDto[]; timezone: string; selectedEventId: string | null; onOpen(event: CalendarEventDto): void; onDropUnit(unitId: string, date: string): void }) {
  ...
  return <div className="calendar-month min-h-0 w-full overflow-hidden rounded-[14px] bg-surface">
    <div className="calendar-weekdays grid grid-cols-7 gap-1 bg-transparent text-center text-[10px] uppercase tracking-[0.12em] text-muted">{DOW.map((day) => <span className="rounded-[8px] bg-surface-sunken py-2" key={day}>{day}</span>)}</div>
    <div className="calendar-month-grid mt-1 grid grid-cols-7 gap-1 bg-transparent">{days.map((day) => {
      const items = events.filter((event) => event.at !== null && calendarDayKey(event.at, timezone) === day.key);
      return <div className={`calendar-month-cell min-h-24 min-w-0 overflow-hidden rounded-control border-0 bg-surface-sunken p-1.5 text-[11px] text-ink shadow-none${day.inMonth ? "" : " is-outside opacity-45"}${day.key === today ? " is-today bg-surface" : ""}`} key={day.key} tabIndex={0} onDragOver={(event) => { if (event.dataTransfer.types.includes(CALENDAR_UNIT_DRAG)) event.preventDefault(); }} onDrop={(event) => { const unitId = event.dataTransfer.getData(CALENDAR_UNIT_DRAG); if (unitId) onDropUnit(unitId, day.key); }}>
