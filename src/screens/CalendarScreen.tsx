...
export function CalendarScreen({
  ...
  return <CalendarWorkspaceContext.Provider value={workspaceId}><InstrumentScreenRoot descriptor={calendarInstrumentStates} state={instrumentState}><main className="main-region calendar-region flex min-h-0 flex-1 flex-col overflow-auto bg-transparent p-2 pb-6 type-base text-ink">
    <section className="calendar-shell m-0 flex min-h-0 w-full max-w-none flex-1 flex-col gap-2 overflow-visible bg-transparent p-0" aria-busy={loading} aria-label={`${workspaceName} calendar`}>
      <header className="calendar-toolbar m-0 flex min-h-0 w-full max-w-none flex-wrap items-center gap-2 rounded-panel bg-instrument px-4 py-3 text-on-instrument [&_button]:inline-flex [&_button]:min-h-8 [&_button]:items-center [&_button]:gap-1.5 [&_button]:rounded-control [&_button]:px-2.5 [&_button]:type-sm">
        ...
        <strong className="min-w-32 type-base font-medium text-on-instrument">{title}</strong><i className="flex-1" />
        ...
        <button type="button" className={rightPanel === "drawer" ? "is-active bg-surface text-ink" : "text-on-instrument-muted"} onClick={() => setRightPanel((panel) => panel === "drawer" ? null : "drawer")}><PanelRight />Ready to schedule <small>{data?.readyUnits.length ?? 0}</small></button>
        <button type="button" className="calendar-primary bg-surface text-ink" onClick={() => openSchedule()}><Plus />Schedule content</button>
      </header>

      <div className="calendar-subbar m-0 flex min-h-10 w-full flex-wrap items-center gap-2 rounded-panel bg-surface px-3 py-2 type-xs text-muted">
      ...
      {!data?.postiz.available && data && <div className="calendar-readonly m-0 flex min-h-10 items-center gap-2 rounded-control bg-surface-sunken px-3 py-2 type-sm text-muted"><CircleAlert className="shrink-0" />Postiz is unavailable. Your local calendar and drafts are still available.<button type="button" onClick={() => setRefresh((value) => value + 1)}>Try again</button></div>}
      <div className="calendar-content m-0 min-h-0 w-full max-w-none flex-1 overflow-visible bg-transparent p-0">
...
function FilterPopover({ data, filters, onChange, onClose }: { data: CalendarWorkspaceDto | null; filters: CalendarFilters; onChange(value: CalendarFilters): void; onClose(): void }) {
  ...
  return <div className="calendar-filter-popover rounded-cell bg-surface p-2 text-ink" data-instrument-overlay="calendar-filter">
...
function FilterOptions({ title, values, selected, onChange }: { title: string; values: string[][]; selected: string[]; onChange(values: string[]): void }) {
  return <section><small>{title}</small>{values.map(([value, label]) => <button type="button" key={value} className={selected.includes(value!) ? " is-selected bg-instrument text-on-instrument" : "bg-transparent text-muted"} onClick={() => onChange(selected.includes(value!) ? selected.filter((item) => item !== value) : [...selected, value!])}><span>{label}</span>{selected.includes(value!) && <Check />}</button>)}</section>;
}
...
function MonthView({ days, events, timezone, selectedEventId, onOpen, onDropUnit }: { days: ReturnType<typeof monthDays>; events: CalendarEventDto[]; timezone: string; selectedEventId: string | null; onOpen(event: CalendarEventDto): void; onDropUnit(unitId: string, date: string): void }) {
  ...
  return <div className="calendar-month min-h-0 w-full overflow-hidden bg-transparent">
    <div className="calendar-weekdays grid grid-cols-7 gap-1.5 bg-transparent text-center type-meta uppercase tracking-mono text-muted">{DOW.map((day) => <span className="py-1.5 pl-0" key={day}>{day}</span>)}</div>
    <div className="calendar-month-grid mt-1.25 grid grid-cols-7 gap-1.5 bg-transparent">{days.map((day) => {
      const items = events.filter((event) => event.at !== null && calendarDayKey(event.at, timezone) === day.key);
      return <div className={`calendar-month-cell min-h-24 min-w-0 overflow-hidden rounded-cell p-1.75 type-xs text-ink ${day.inMonth ? " bg-surface" : " is-outside bg-transparent"}${day.key === today ? " is-today bg-surface-hover" : ""}`} key={day.key} tabIndex={0} onDragOver={(event) => { if (event.dataTransfer.types.includes(CALENDAR_UNIT_DRAG)) event.preventDefault(); }} onDrop={(event) => { const unitId = event.dataTransfer.getData(CALENDAR_UNIT_DRAG); if (unitId) onDropUnit(unitId, day.key); }}>
...
function WeekView({ days, events, timezone, selectedEventId, onOpen }: { days: ReturnType<typeof weekDays>; events: CalendarEventDto[]; timezone: string; selectedEventId: string | null; onOpen(event: CalendarEventDto): void }) {
  ...
  const today = localDateKey(new Date());
  return <div className="calendar-week min-h-0 w-full overflow-hidden rounded-cell bg-surface-sunken type-sm text-ink">
...
function CalendarEventButton({ event, timezone, lead = false, selected = false, onClick }: { event: CalendarEventDto; timezone: string; lead?: boolean; selected?: boolean; onClick(): void }) {
  return <button type="button" className={`calendar-event flex w-full min-w-0 items-center gap-1 overflow-hidden rounded-full px-1.5 py-1 text-left type-xs leading-4 ${lead ? " is-lead" : ""}${selected ? " is-selected bg-instrument text-on-instrument [&_small]:text-on-instrument-muted" : " bg-surface text-ink"} is-${event.status}`} onClick={onClick}>{lead && <CalendarThumb event={event} className="h-9 w-[30px] rounded-full" />}<span className="min-w-0"><b className="block truncate font-medium">{event.title}</b><small className="block truncate type-mono-xs text-muted"><i />{formatCalendarTime(event.at, timezone)} · {event.channels.length} {event.channels.length === 1 ? "publication" : "publications"}</small></span>{eventStatusSummary(event) === "attention" && <AlertTriangle />}</button>;
}
...
function EventInspector({ event, postizAvailable, onClose, onOpenUnit, onMutate }: { event: CalendarEventDto; postizAvailable: boolean; onClose(): void; onOpenUnit(): void; onMutate(input: Parameters<typeof bridge.mutateCalendar>[1]): void }) {
  return <aside className="calendar-inspector rounded-panel bg-surface text-ink [&_.calendar-date-card]:bg-surface-sunken [&_.calendar-event-summary]:bg-surface-sunken [&_.calendar-revision-card]:bg-surface-sunken" data-instrument-overlay="calendar-inspector">
  ...
  return <Dialog.Root open={open} onOpenChange={onOpenChange}>{open && <><Dialog.Overlay forceMount className="calendar-modal-overlay" data-instrument-overlay-backdrop="" /><Dialog.Content forceMount className="calendar-modal rounded-panel bg-surface text-ink [&_input]:bg-surface-sunken [&_textarea]:bg-surface-sunken" data-instrument-overlay="calendar-schedule">
    ...
    <div className="calendar-modal-layout">
      <aside className="calendar-modal-unit">
        ...
        <section><label>REVISION TO PUBLISH</label><div className="calendar-revision-picker">{revisions.map((revision) => <button type="button" className={revision.unitRevisionId === revisionId ? "is-active bg-instrument text-on-instrument" : "bg-surface-sunken text-muted"} key={revision.unitRevisionId} onClick={() => setRevisionSelection({ unitId: unit!.unitId, revisionId: revision.unitRevisionId })}>R{revision.revision}</button>)}</div>{activeRevision && latestRevision !== null && activeRevision.revision < latestRevision ? <small className="is-warning"><AlertTriangle />R{activeRevision.revision} is older than the latest R{latestRevision}. This publication will stay pinned to R{activeRevision.revision}.</small> : <small>R{activeRevision?.revision ?? unit?.revision ?? "—"} is pinned to this publication. Calendar keeps it when the Unit selection changes.</small>}</section>
        <button type="button" className="calendar-pick-unit" onClick={() => setPickerOpen((value) => !value)}><Repeat />Pick another unit</button>
        {pickerOpen && <div className="calendar-unit-popover rounded-cell bg-surface-sunken" data-instrument-overlay="calendar-unit-picker">{units.map((item) => <button type="button" key={item.unitId} className={item.unitId === unit?.unitId ? " is-selected bg-instrument text-on-instrument [&_small]:text-on-instrument-muted" : "bg-transparent text-ink"} onClick={() => { onSelect(item); setPickerOpen(false); }}><CalendarThumb event={item} /><span><strong>{item.title}</strong><small>{item.project} · R{item.revision ?? "—"} · {item.kind}</small></span>{item.unitId === unit?.unitId && <Check />}</button>)}</div>}
      </aside>
      {step === "content" ? <section className="calendar-schedule-form">
        ...
        <section className="calendar-channel-section"><header><span>CHANNELS</span><small>{selectedChannels.length} of {channels.filter((channel) => !disconnectedIds.has(channel.socialAccountId)).length} available selected{channels.some((channel) => disconnectedIds.has(channel.socialAccountId)) ? ` · ${channels.filter((channel) => disconnectedIds.has(channel.socialAccountId)).length} needs reconnect` : ""}</small><i /><button type="button" disabled={!unit} onClick={() => onStep("settings")}><SlidersHorizontal />Platform settings</button></header><div className="calendar-modal-channels">{channels.map((channel) => { const id = channelKey(channel); const Icon = platformIcon(channel.platform); const disconnected = disconnectedIds.has(channel.socialAccountId); const selected = selectedIds.includes(id); const content = <><AccountMark identity={`${channel.platform}:${channel.account}`} /><span><strong><Icon />{channel.account}</strong><small>{disconnected ? "Token expired — reconnect to publish" : `${capitalize(channel.platform)} · ready`}</small></span>{disconnected ? <button type="button" onClick={() => onReconnect(channel.socialAccountId)}>Reconnect</button> : <i className="calendar-check-box">{selected && <Check />}</i>}</>; return disconnected ? <div className="calendar-channel-option is-disconnected bg-surface-sunken" key={id}>{content}</div> : <button type="button" className={`calendar-channel-option ${selected ? " is-selected bg-instrument text-on-instrument [&_small]:text-on-instrument-muted" : " bg-surface-sunken text-ink"}`} key={id} onClick={() => toggleChannel(id)}>{content}</button>; })}</div></section>
    ...
    <footer className=" bg-surface-sunken"><small>{step === "content" ? "Published through Postiz — selected accounts leave as one publication. Channel staggering lives in Platform settings." : "Settings belong to this publication, not the account. The next publication starts from platform defaults."}</small><span>{step === "content" ? <><button type="button" disabled={!unit || saving || selectedChannels.length === 0} onClick={() => onSave(false, at(), payload(), revisionId)}>Save as draft</button><button type="button" className="calendar-primary bg-instrument text-on-instrument" disabled={!unit || !postizAvailable || saving || selectedChannels.length === 0} onClick={() => onSave(true, at(), payload(), revisionId)}>{saving ? "Saving…" : `Schedule ${selectedChannels.length} ${selectedChannels.length === 1 ? "publication" : "publications"}`}</button></> : <><button type="button" onClick={() => { setSettings(settingsForChannels(channels)); setEditedIds(new Set()); }}>Reset to defaults</button><button type="button" className="calendar-primary bg-instrument text-on-instrument" onClick={() => onStep("content")}><ArrowLeft />Back to schedule</button></>}</span></footer>
...
function CalendarDatePicker({ value, onChange, onClose }: { value: string; onChange(value: string): void; onClose(): void }) {
  ...
  const today = localDateKey(new Date());
  return <div className="calendar-date-popover rounded-cell bg-surface text-ink" data-instrument-overlay="calendar-date-popover" role="dialog" aria-label="Publication date" ref={ref} tabIndex={-1} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); onClose(); } }}>
    ...
    <div className="calendar-picker-days">{days.map((day) => <button type="button" className={`${day.inMonth ? "" : "is-outside"}${day.key === value ? " is-selected bg-instrument text-on-instrument" : ""}${day.key === today ? " is-today" : ""}`} aria-label={`Choose ${new Intl.DateTimeFormat("en", { dateStyle: "full" }).format(day.date)}`} key={day.key} onClick={() => onChange(day.key)}>{day.date.getDate()}</button>)}</div>
...
function CalendarTimePicker({ value, onChange, onClose }: { value: string; onChange(value: string): void; onClose(): void }) {
  ...
  return <div className="calendar-time-popover rounded-cell bg-surface text-ink" data-instrument-overlay="calendar-time-popover" role="dialog" aria-label="Publication time" ref={ref} tabIndex={-1} onKeyDown={(event) => { if (event.key === "Escape") { event.stopPropagation(); onClose(); } }}>
    <header><strong>TIME</strong><small>24 HOUR</small></header>
    <div><section aria-label="Hours">{Array.from({ length: 24 }, (_, item) => <button type="button" className={item === hour ? " is-selected bg-instrument text-on-instrument" : "bg-transparent text-muted"} aria-label={`Set hour ${String(item).padStart(2, "0")}`} ref={item === hour ? selectedHour : undefined} key={item} onClick={() => setHour(item)}>{String(item).padStart(2, "0")}</button>)}</section><section aria-label="Minutes">{Array.from({ length: 12 }, (_, item) => item * 5).map((item) => <button type="button" className={item === minute ? " is-selected bg-instrument text-on-instrument" : "bg-transparent text-muted"} aria-label={`Set minute ${String(item).padStart(2, "0")}`} key={item} onClick={() => setMinute(item)}>{String(item).padStart(2, "0")}</button>)}</section></div>
  </div>;
}
...
function ReconnectDialog({ account, credential, saving, onCredential, onOpenChange, onSave }: { account: CalendarWorkspaceDto["accounts"][number] | null; credential: string; saving: boolean; onCredential(value: string): void; onOpenChange(open: boolean): void; onSave(): void }) {
  return <Dialog.Root open={account !== null} onOpenChange={onOpenChange}>{account && <><Dialog.Overlay forceMount className="calendar-reconnect-overlay" data-instrument-overlay-backdrop="" /><Dialog.Content forceMount className="calendar-reconnect-dialog rounded-panel bg-surface text-ink" data-instrument-overlay="calendar-reconnect">
...
function SettingsSegment({ label, options, value, onChange }: { label: string; options: string[]; value: string; onChange(value: string): void }) { return <label className="calendar-settings-field"><span>{label}</span><span className="calendar-settings-segment">{options.map((option) => <button type="button" className={value === option ? "is-active bg-instrument text-on-instrument" : " bg-surface-sunken text-ink"} key={option} onClick={() => onChange(option)}>{option}</button>)}</span></label>; }
