...
The evidence manifest advances only through `scenario` → `accessibility-complete` → `media-complete` → `final`. The scenario phase permits incomplete journey collection and null required Media pixel evidence while validating every stored entry. The accessibility transition requires the exact per-case journey sets and exact global `system-theme` suite but deliberately permits the two required 1440×900 light/dark Media records to remain null or partially attached. Only the Media transition requires complete pixel evidence for both exact required keys and rejects pixel evidence elsewhere; finalization revalidates accessibility, Media, launch/DB/artifact links, and all reviewer decisions.

`updateEvidenceBundle(manifestPath, expectedRevision, update)` is the only evidence mutation API used by scenario, accessibility, Media, and final producers. Each call acquires an exclusive sibling lock, rejects a stale revision, applies one typed record-plus-ledger/artifact update or adjacent phase transition to a validated clone, increments the revision, validates the resulting phase, writes and flushes a same-directory temporary file, atomically renames it over the manifest, and flushes the directory before releasing the lock. A failed apply, validation, or write leaves the prior valid manifest bytes unchanged; scripts never patch the JSON, launch ledger, or artifact links directly. This lets Task 12 publish a valid `accessibility-complete` bundle without Media evidence and Task 13 append each Media capture safely before atomically advancing it to `media-complete`.
