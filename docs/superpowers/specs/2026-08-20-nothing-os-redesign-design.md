...
- Settings search, sticky header, Back to app, modals, menus, shared selects, context menus, tooltips, and profile menu.
...
`src/instrument/overlay-registry.tsx` is the production overlay authority. `InstrumentOverlayId` is derived from its object keys, never duplicated as a hand-written union, and its `InstrumentOverlay` wrapper is used by every live dialog, drawer, viewer, menu, sheet, popover, and portalled listbox. In addition to root/global overlays; workspace account, Unit outcome, and evidence details; Shared inspector/viewer/workflow; Memory recall/editor/history/confirm; Calendar filter/drawer/inspector/schedule/unit/date/time/platform/account/reconnect surfaces; Document editor/viewer/conflict; Media viewer/context/mock Needs Work; Unit viewer; run inspector; Marketplace detail/target chooser; and terminal, the registry has exact IDs for `shared-select-menu`, `agent-chat-recent-menu`, `agent-chat-provider-menu`, `agent-chat-model-menu`, `agent-chat-mode-menu`, and `workspace-picker`. `SelectMenu`, the four menu components in `UtilityPanels`, and `WorkspacePicker` are the only approved primitive-host adapters: each renders `InstrumentOverlay` with `data-instrument-overlay`, while preserving its existing value/selection, anchoring/positioning, keyboard, dismissal, and opener-focus API. The shared-select registration names every production import owner and applicable route; route scenarios cover each registered owner with the listbox open. Chat has one open-state scenario/journey per exact menu ID, and the workspace picker has open/search/select/Escape/focus-return coverage on every route where the production sidebar exposes it. A reachable raw overlay implementation or primitive/import site absent from this production authority is an audit failure.

Each production route component exports a typed applicable-state descriptor and renders through `InstrumentScreenRoot` with its current state. `PRODUCTION_SCREEN_STATES` imports those owner exports. `INSTRUMENT_SCENARIOS` derives its requirements from the real route unions, those production descriptors, and the production overlay registry, including the registry-derived shared-overlay owner IDs. Completeness compares route/state pairs, overlay keys, and applicable `(routeKey, overlayId, overlayOwner)` tuples in both directions; the source audit independently enumerates reachable `InstrumentOverlay` uses, approved primitive adapters, and their import sites. Omitting a live state/overlay/owner from production registration or scenario evidence, or registering an owner with no reachable source site, fails type/tests/source audit.
...
Scenario fixtures are deterministic renderer-only modules loaded only when `VITE_RALPHY_ENABLE_MOCKS === "true"`; production bundles contain neither fixture IDs nor fixture chunk paths. Real Electron evidence is captured per scenario, not just per shell permutation, into a versioned ignored bundle with build/app/Core hashes, run mode, viewport/theme, native and content geometry, landmarks, typed measurements, focus/scroll/contrast results, artifacts/logs, an `accessibilityJourneys` array, pixel-diff data, DB record link, launch/exit result, failures, and product/accessibility/security/regression reviewer decisions. Each scenario-case record remains unique by `(mode, scenarioId, theme, viewport)` and contains exactly one accessibility entry for every case journey declared by that scenario: `keyboard`, `reduced-motion`, or `live-region`. Each entry records its journey ID/kind, steps/focus/live-region/motion result, artifact/log paths, distinct Electron-child launch/exit result, launch-ledger ID, and DB record link. System theme is not a scenario-case journey: the manifest has an exact typed global accessibility suite containing one `system-theme` execution with steps `launch-system-dark`, `switch-os-light`, and `assert-root-consumers`, its own isolated-profile Electron child, artifacts/logs, exit result, launch-ledger ID, DB record, and scoped `reviewer.accessibility` sign-off. That field starts null and must contain a non-empty `approved: ...` decision before final acceptance. Its ledger entry uses `scenarioEvidenceKey: null`; validators compare both the per-case declared journey sets and the exact global ID set in both directions. The generated HTML displays per-case and Global Accessibility sections separately, including the global sign-off. The launch ledger links every scenario, case journey, global journey, Electron, or reference-browser child to main DB/WAL verification and a separate SHM observation. Schema validation and the generated report require every field, reject missing/extra/duplicate journeys and broken ledger/DB/artifact links, and never overwrite a scenario child with a journey child. Mock and production names never collide.
...
The evidence manifest advances only through `scenario` → `accessibility-complete` → `media-complete` → `final`. The scenario phase permits incomplete journey collection and null required Media pixel evidence while validating every stored entry. The accessibility transition requires the exact per-case journey sets and exact global `system-theme` suite but permits its reviewer sign-off to remain pending and the two required 1440×900 light/dark Media records to remain null or partially attached. Only the Media transition requires complete pixel evidence for both exact required keys and rejects pixel evidence elsewhere; finalization revalidates accessibility, Media, launch/DB/artifact links, all scenario-record reviewer decisions, and the non-null global `system-theme` accessibility sign-off.

`updateEvidenceBundle(manifestPath, expectedRevision, update)` is the only evidence mutation API used by scenario, accessibility, Media, and final producers. Its typed update union includes the `system-theme`-only `set-global-accessibility-reviewer` operation, so the global sign-off never masquerades as a scenario-record decision. Each call acquires an exclusive sibling lock, rejects a stale revision, applies one typed record-plus-ledger/artifact/reviewer update or adjacent phase transition to a validated clone, increments the revision, validates the resulting phase, writes and flushes a same-directory temporary file, atomically renames it over the manifest, and flushes the directory before releasing the lock. A failed apply, validation, or write leaves the prior valid manifest bytes unchanged; scripts never patch the JSON, launch ledger, artifact links, or reviewer fields directly. This lets Task 12 publish a valid `accessibility-complete` bundle without Media evidence, record its scoped global review atomically, and Task 13 append each Media capture safely before atomically advancing it to `media-complete`.
...
- Automated keyboard journeys cover sidebar, the workspace picker portal, dock, island, profile/Settings menus, every shared Select owner, all four Agent Chat menus, filters/cards, sheets/dialogs/viewers, Escape, and focus restoration. Live-region tests prove polite deduplication and that island/chat updates never move focus.
