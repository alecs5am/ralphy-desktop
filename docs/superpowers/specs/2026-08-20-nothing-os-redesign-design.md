...
The evidence manifest advances only through `scenario` → `accessibility-complete` → `final`: empty journey arrays are permitted only during scenario collection, the accessibility transition validates the exact declared journey set for every case, and finalization revalidates those links before reviewer acceptance.
