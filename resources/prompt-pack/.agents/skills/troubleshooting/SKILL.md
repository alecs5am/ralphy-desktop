...
description: >-
  ...
  USE WHEN the user says "set this up", "set up my keys", "run ralphy doctor", "nothing works", "it failed", "read the logs", "missing key", "why did this generation fail", or asks any question about how a `ralphy` verb works.
  TRIGGER (EN): "doctor", "nothing works", "set up keys", "read the logs", "debug this failure", "missing API key", "how do I run <verb>".
---
...
|---|---|
| [core/doctor.md](./references/doctor.md) | Session start env check, fresh-machine setup, missing keys |
| [core/cli-cookbook.md](./references/cli-cookbook.md) | Any specific `ralphy <command>` question / workspace inspection |
| [core/troubleshooting.md](./references/troubleshooting.md) | Failed generation, "what was in the last prompt", log reading |
...
1. **NO auto-launch.** I don't run a UI or scheduler in the background. Chat is the interface. See [core/doctor.md](./references/doctor.md).
...
4. **Logs append-only.** `cli/lib/gen-log.ts` enforces the format. See [core/troubleshooting.md](./references/troubleshooting.md).
...
- HyperFrames-specific → **[hyperframes playbook](../hyperframes/references/playbook.md)** (not me).
