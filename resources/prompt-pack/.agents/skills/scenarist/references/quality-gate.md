...
7. **Persona/brand reference policy** — if a scene contains a named persona/brand without `image_urls` → fail with a pointer to [`../art-director/ref-photo-policy.md`](../../art-director/references/ref-photo-policy.md) (the art-director handoff will require the ref anyway).
